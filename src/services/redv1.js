    ...
    try {
        ...
        followersSnapshot.forEach((doc) => {
            ...
            if (Array.isArray(userData.followers)) {
                // Filtrar seguidores válidos y agregarlos a la lista
                const validFollowers = userData.followers.filter((followerId) => {
                    return followerId !== null && followerId !== undefined;
                });

                validFollowers.forEach((followerId) => {
                    followers.push({
                        id: followerId,
...
export async function getFollowing(userId) {
    try {
        ...
        followingSnapshot.forEach((doc) => {
            const userData = doc.data();
            // Verificar si 'followers' es un array antes de usar 'indexOf'
            if (Array.isArray(userData.followers) && Array.isArray(userData.following)) {
                if (userData.followers.includes(userId) && userData.following.indexOf(userId) !== -1) {
                    following.push({
                        id: doc.id,
                        displayName: userData.displayName,
                    });
